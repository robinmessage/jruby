...
public class SSLSocket extends RubyObject {
    ...
    public IRubyObject _initialize(IRubyObject[] args) throws Exception {
        ...
        if(checkArgumentCount(args,1,2) == 1) {
            ctx = ((RubyModule)(getRuntime().getModule("OpenSSL").getConstant("SSL"))).getClass("SSLContext").callMethod("new");
        } else {
            ctx = args[1];
        }
        io = args[0];
        ...
        callMethod("context=",ctx);
        callMethod("sync_close=",getRuntime().getFalse());
        return callMethod(getMetaClass().getSuperClass(),"initialize",args,CallType.SUPER);
    }
    ...
    private void ossl_ssl_setup() throws Exception {
        if(null == engine) {
            ...
            if(store.isNil()) {
                ctx.init(new javax.net.ssl.KeyManager[]{((org.jruby.ext.openssl.SSLContext)callMethod("context")).getKM()},new javax.net.ssl.TrustManager[]{((org.jruby.ext.openssl.SSLContext)callMethod("context")).getTM()},null);
            } else {
                ctx.init(new javax.net.ssl.KeyManager[]{((org.jruby.ext.openssl.SSLContext)callMethod("context")).getKM()},new javax.net.ssl.TrustManager[]{((X509Store)store).getStore()},null);
            }
            ...
            engine.setEnabledCipherSuites(((org.jruby.ext.openssl.SSLContext)callMethod("context")).getCipherSuites(engine));
    ...
    public IRubyObject sysclose() throws Exception {
        ...
        close();
        if(callMethod("sync_close").isTrue()) {
            c.close();
            callMethod("io").callMethod("close");
        }
